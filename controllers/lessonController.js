...
class LessonController {
     async add(req, res, next) {
          try {
               ...
               const checkCourseId = await prisma.course.findFirst({ where: { id: Number(course_id) } })
               ...
               const checkAvailability = await prisma.lesson.findFirst({ where: { title } })
     ...
     async update(req, res, next) {
          try {
               ...
               const checkId = await prisma.lesson.findFirst({ where: { id: Number(id) } })
               ...
               if (title) {
                    ...
                    if (checkTitle) {
                         return next(ApiError.badRequest(`урок с таким названием уже существует`))
                    }
               }
     ...
     async getById(req, res, next) {
          try {
               ...
               const lesson = await prisma.lesson.findFirst({ where: { id: Number(id) } })
